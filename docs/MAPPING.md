...
## Grid Allocation (Hypothesis & Findings)
...
| **6**      | `0x0C`         | **d1**        | Almost full digit. Missing 'd' segment. |
...
| **12**     | `0x18`         | **d6**?       | *To be mapped* |
| **13**     | `0x1A`         | **d7**?       | *To be mapped* |
| **14**     | `0x1C`         | **d8**?       | *To be mapped* |
| **15**     | `0x1E`         | Unknown       | *To be mapped* |
...
- **p**: Previous Bit
- **N / P**: Next / Previous Grid (0-15)
- **jump <grid> [bit]**: Jump to specific location (e.g., `jump 6 0`)
- **map <name>**: Record mapping for current segment (e.g., `map d1_d` or `map play`)
...
1.  **d1 segment 'd'** (Bottom horizontal line). Likely on Grid 5 or 7.
2.  **Digits d4 - d8**. Likely on Grids 10-15.
